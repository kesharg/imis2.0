...
{{--            </div>--}}
            <div class="form-group row">
                ...
                <div class="col-md-3">
                    ...
                    null,
                    ['class' => 'form-control', 'placeholder' => '--- Select a Format ---'])
                    !!}
                </div>
            </div>
        </div><!-- /.card-body -->
...
$(document).ready(function(){
    $('#ward, #ward_overlay, #taxzone').multipleSelect({
        filter: true
    });
    ...
    $('#export_form').on('submit', function() {
        ...
        if(checkValidation(selectedWards, selectedLayers, selectedFormat))
        {
            ...
            var exportLink = gurl_wfs + "?request=GetFeature&service=WFS&version=1.0.0&authkey={{ Config::get("constants.AUTH_KEY") }}&typeName=" + layers.join(',') + "&CQL_FILTER=INTERSECTS(geom, collectGeometries(queryCollection('" + workspace + ":wards_layer', 'geom', '" + filters.join(' OR ') + "')))&outputFormat=" + outputFormat;
