...
use App\Models\Fsm\KpiAchievement;
                    ...
                    FROM
                        ...
                        ON
                            emptyings.application_id IN (
                                ...
                                WHERE buildings.house_number = a.house_number and EXTRACT(year FROM a.application_date) = $select_year and  $whereServiceProviderID
                            )
                        GROUP BY
                ...
                foreach($quarters as $quarter)
                {
                    ...
                    $inclusion = DB::select(DB::raw("SELECT SUM(application_count) AS total_application_count
                        FROM (SELECT COUNT(emptyings.application_id) AS application_count
                            ...
                            ON emptyings.application_id IN ( SELECT a.id FROM fsm.applications AS a WHERE buildings.house_number = a.house_number and EXTRACT(year FROM a.application_date) = $select_year and $whereServiceProviderID  AND (
                ...
                foreach($years as $year)
                    {
                        ...
                        $inclusion = DB::select(DB::raw("SELECT SUM(application_count) AS total_application_count
                            ...
                            fsm.emptyings AS emptyings  ON emptyings.application_id IN ( SELECT a.id FROM fsm.applications AS a
                                WHERE buildings.house_number = a.house_number and EXTRACT(year FROM a.application_date) = $year and $whereServiceProviderID
                            ) GROUP BY buildings.geom) AS subquery; "));
                        $inclusionValue = $inclusion[0]->total_application_count;
