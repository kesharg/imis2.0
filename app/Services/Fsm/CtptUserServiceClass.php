...
class CtptUserServiceClass
{
        ...
        return Datatables::of($cwis_mof)
            ...
            ->addColumn('action', function ($model) {
                ...
                if (Auth::user()->can('Edit Male or Female User')) {
                    $content .= '<a title="Edit Male or Female User" href="' . action("Fsm\CtptUserController@edit", [$model->id]) . '" class="btn btn-info btn-sm mb-1"><i class="fa fa-edit"></i></a> ';
                }

                if (Auth::user()->can('View Male or Female User')) {
                    $content .= '<a title="View Male or Female User" href="' . action("Fsm\CtptUserController@show", [$model->id]) . '" class="btn btn-info btn-sm mb-1"><i class="fa fa-list"></i></a> ';
                }
                ...
                if (Auth::user()->can('Delete Male or Female User')) {
                    $content .= '<a title="Delete Male or Female User"  class="delete btn btn-danger btn-sm mb-1"><i class="fa fa-trash"></i></a> ';
                }
                $content .= \Form::close();
                return $content;
            })
    ...
    public function exportData()
    {
        ...
        $date = $_GET['date'] ?? null;
        $columns = ['ID','Toilet Name', 'No. of Male Users (daily)', 'No. of Female Users (daily)', 'Date'];
        $query = DB::table('fsm.ctpt_users as ctpt')
            ->leftJoin('fsm.toilets as t', 'ctpt.toilet_id', '=', 't.id')
            ->select('ctpt.id as id',
            't.name as name',
            'ctpt.toilet_id',
             'ctpt.no_male_user as no_of_male_user', 'ctpt.no_female_user as no_of_female_user', 'ctpt.date as date')
        ...
        $writer->openToBrowser('PT Users Log.CSV')
            ...
            $query->chunk(5000, function ($ctpt_users) use ($writer) {
                ...
                foreach($ctpt_users as $ctpt_user) {
                    ...
                    $values[] = $ctpt_user->no_of_male_user;
                    $values[] = $ctpt_user->no_of_female_user;
